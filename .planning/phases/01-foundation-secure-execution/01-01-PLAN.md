...
<task type="auto">
  ...
  <action>
    ...
    - Create `requirements.txt` with: `pydantic-ai`, `open-interpreter`, `docker`, `sqlalchemy`, `aiosqlite`, `pydantic-settings`.
...
<task type="auto">
  ...
  <files>src/sandbox/languages.py, Dockerfile</files>
  <action>
    ...
    - Implement `src/sandbox/languages.py`:
      - `DockerPython`: Implements the Open Interpreter Language interface. Routes `run(code)` to `container.exec_run(["python3", "-c", code])`.
      - `DockerShell`: Routes commands to `container.exec_run(["/bin/bash", "-c", command])`.
    - Handle state persistence: For Python, consider using a persistent process or clear documentation that state is file-based for now.
    - Ensure output (stdout/stderr) is captured and yielded in the format expected by Open Interpreter.
  </action>
  <verify>
    <automated>pytest -v (Note: Create a small test script to verify DockerShell can run 'echo hello' and return it)</automated>
    <manual>N/A</manual>
    <sampling_rate>once after implementation</sampling_rate>
  </verify>
