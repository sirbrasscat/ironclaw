...
<task type="auto">
  ...
  <verify>
    <automated>python3 -c "from src.agent.core import ironclaw_agent; assert any(t.function.__name__ == 'run_system_task' for t in ironclaw_agent._tools.values()); print('Tool registered')"</automated>
    <manual>N/A</manual>
...
<task type="auto">
  ...
  <files>src/agent/tools/sandbox.py, tests/test_hitl.py</files>
  <action>
    ...
    - Create `tests/test_hitl.py`:
      - Verify that calling `run_system_task` returns a 'Pending' status and the code to be run.
      - Verify that execution does not proceed until `confirm_execution` (or equivalent mechanism) is triggered.
  </action>
  <verify>
    <automated>pytest tests/test_hitl.py</automated>
    <manual>N/A</manual>
    <sampling_rate>once after implementation</sampling_rate>
  </verify>
